...
class DataProcessing extends Model
{
	...
	public function run($arrOffers = array())
	{
		...
		switch ($this->type)
		{
			case 'xml':
				switch ($this->scope)
				{
					case 'full':
						...
						{
							$arrToExport[self::EXPORT_OFFER_IMAGES_PATH] = $objMountManger->listContents('dbafs://' . $objOfferImages->path);
						}
						if (null !== ($objHostLogos = \FilesModel::findByPk($this->host_logo_path)))
						{
							$arrToExport[self::EXPORT_HOST_LOGOS_PATH] = $objMountManger->listContents('dbafs://' . $objHostLogos->path);
						}
						break;
				...
				if (null !== $objOffers)
				{
					...
					while ($objOffers->next())
					{
						...
						if (false !== $strXml)
						{
							...
							$arrToExport[self::EXPORT_XML_FILES_PATH] = $objMountManger->listContents('local://' . $strTmpPath);
						}
					}
				}
				break;
			...
			case 'ical':
				...
				$arrToExport[] = array_merge($objMountManger->getMetadata('local://' . $path), array('basename' => basename($path)));
				break;

			default:
				throw new \LogicException(sprintf('Type "%s" is not implemented. Data processing ID %u', $this->type, $this->id));
				break;
		}
		...
		switch ($this->filesystem)
		{
			...
			case 'local':
				...
				if (array_is_assoc($arrToExport))
				{
					foreach ($arrToExport as $directory => $arrFiles)
					{
						...
						{
							$objMountManger->put('local://share/' . $path_prefix . $directory . '/' . $file['basename'], $objMountManger->read('local://' . $file['path']));
						}
					}
				}
				else
				{
					...
					{
						$objMountManger->put('local://share/' . $path_prefix . $file['basename'], $objMountManger->read('local://' . $file['path']));
					}
				}
				break;
			...
			case 'dropbox':
				...
				foreach ($arrToExport as $directory => $arrFiles)
				{
					foreach ($arrFiles as $file)
					{
						try
						{
							$objMountManger->put('dropbox://' . $directory . '/' . $file['basename'], $objMountManger->read('local://' . $file['path']));

						} catch (Exception_BadRequest $e)
						{
							// File was not uploaded
							// often because it is on the ignored file list
							\System::log(sprintf('%s. Data processing ID %u', $e->getMessage(), $this->id), __METHOD__, TL_GENERAL);
						} catch (Exception_NetworkIO $e)
						{
							// File was not uploaded
							// Connection refused
							\System::log(sprintf('%s. Data processing ID %u', $e->getMessage(), $this->id), __METHOD__, TL_ERROR);
						}
					}
				}
				break;
			...
			default:
				throw new \LogicException(sprintf('Filesystem "%s" is not implemented. Data processing ID %u', $this->filesystem, $this->id));
				break;
		}
			...
			foreach ($arrICalProperties as $arrProperty)
			{
				switch ($arrProperty['ical_field'])
				{
					case 'dtStart':
						try
						{
							$objDate = new DateTime('@' . $objOffers->getItem()->get($arrProperty['metamodel_attribute']));
	...
	public function syncFromRemoteDropbox()
	{
		...
		if (array_key_exists(self::EXPORT_XML_FILES_PATH, $arrFiles))
		{
			...
			$this->syncXmlFilesWithModel(array_map(function ($value)
			{
				return $value[1];
			}, $arrFiles[self::EXPORT_XML_FILES_PATH]), 'dropbox');
		}
	...
	public function generateOfferXml($intId)
	{
		...
		foreach ($objRenderSetting->getSettingNames() as $colName)
		{
			...
			$attribute->nodeValue = htmlspecialchars(html_entity_decode($parsed), ENT_XML1) ?: ' '; // Prohibit empty string
	...
	protected function syncXmlFilesWithModel($arrFiles, $filesystem = 'local')
	{
		...
		foreach ($arrFiles as $file)
		{
			...
			if ($this->combine_variants)
			{
				...
				{
					$objVariants = Offer::getInstance()->findMultipleByIds(trimsplit(',', $objRoot->getAttribute('variant_ids')));
				}
			}
			...
			if ($file['timestamp'] < $objOffer->get('tstamp'))
			{
				...
				\System::log(sprintf('Could not sync XML file "%s" because offer ID %u was edited afterwards', $file['path'], $objOffer->get('id')), __METHOD__, TL_ERROR);
			...
			if ($blnChange)
			{
				...
				{
					$objProcessings->current()->run(array_merge
					(
		...
		switch ($objAttribute->get('type'))
		{
			...
			case 'file':
				...
				foreach ($element->getElementsByTagName('Link') as $file)
				{
					...
					$strPath = preg_replace_callback('/^file:\/\/.*?(' . implode('|', [self::EXPORT_OFFER_IMAGES_PATH, self::EXPORT_HOST_LOGOS_PATH]) . ')/', function ($matches)
					{
